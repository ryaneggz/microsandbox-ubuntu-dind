#!/usr/bin/env bash
# Converge an msb sandbox to running-with-dockerd, then supervise dockerd.
#
# Usage: ensure-sandbox.sh [SANDBOX_NAME]
...
# image ENTRYPOINT/CMD, so dockerd (the image CMD) does not come back on its own.
...
#   missing  -> sandbox.sh         (recreate; ONLY when there is nothing to lose)
# A stopped sandbox is never recreated: that would discard anything written to
# the root disk that is not on a mount or a named volume.
#
...
# msb reports transient lifecycle states while a VM tears down. Acting on them
# races msb's lifecycle lock and aborts with SIGABRT.
wait_for_settled() {
  local i st
  for (( i = 1; i <= 36; i++ )); do
    st="$(sandbox_status)"
  ...
  log "stuck in transient state '$st'"
  return 1
}
...
start_with_retry() {
  ...
  for (( i = 1; i <= START_TRIES; i++ )); do
    ...
    [ "$(sandbox_status)" = "running" ] && { log "already running"; return 0; }

    out="$("$MSB" start "$SANDBOX" 2>&1)"
    printf '%s\n' "$out" | tail -2

    case "$out" in *"already running"*) log "already running"; return 0 ;; esac

    wait_for_running && { log "running (attempt $i)"; return 0; }

    log "start attempt $i/$START_TRIES failed; retrying in ${delay}s"
    sleep "$delay"
    [ "$delay" -lt 60 ] && delay=$(( delay * 2 ))
  done
  log "failed to start after $START_TRIES attempts"
  return 1
}
...
converge_sandbox() {
  ...
  case "$st" in
    running) log "running" ;;
    stopped) log "stopped; starting (non-destructive)"; start_with_retry || return 1 ;;
    "")      log "MISSING; recreating via sandbox.sh"
             ( cd "$REPO" && SANDBOX_NAME="$SANDBOX" ./sandbox.sh ) \
               || { log "sandbox.sh failed"; return 1; } ;;
    draining|starting|stopping)
             log "transient state '$st'; waiting to settle"
